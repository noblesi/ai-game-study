...
def level_up_unit_menu(units: List[Unit]):
    ...
    print("\n[유닛 레벨 올리기]")
    target_name = input("레벨을 올릴 유닛 이름을 입력하세요: ").strip()
    matched_units = find_units_by_name(units, target_name)

    if not matched_units:
        print("해당 이름을 가진 유닛을 찾을 수 없습니다.\n")
        return

    unit = choose_unit_from_list(matched_units)
    if unit is None:
        return
    
    unit.level_up()
    print(f"[완료] {unit.name}의 레벨이 {unit.level}로 올랐습니다.\n")
...
def remove_unit_menu(units: List[Unit]):
    ...
    print("\n[유닛 삭제하기]")
    target_name = input("삭제할 유닛 이름을 입력하세요: ").strip()
    matched_units = find_units_by_name(units, target_name)
    ...
    units.remove(unit)
    print(f"[완료] {unit.name} 유닛을 삭제했습니다.\n")
